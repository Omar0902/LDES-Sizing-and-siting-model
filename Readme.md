...
### `env/`
This directory contains the environment for running the above scripts. However, we recommend only using this for Linux machines. For Windows, we recommend looking at the `Project.toml` file and manually adding the specific package versions to your machine and then marking PowerSimulations.jl and StorageSystemsSimulations.jl for development from the `packages/` directory. This environment can be activated and then instantiated to get access to the required packages. To activate and instantiate a Julia environment, a user can go to the Julia REPL, type `]` to enter the package manager and then type `activate path_to_env_directory/`, hit enter, and then type `instantiate`. Note that we used Julia 1.9.2 for the simulations, run on , and other versions of Julia may not instantiate without error. The exact package versions we used can also be accessed in the file `env/Project.toml`. 