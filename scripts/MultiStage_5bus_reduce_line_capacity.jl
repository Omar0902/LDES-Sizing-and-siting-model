...
# function takes sim_name, bus (for LDES location), and tf (factor for reducing line rating; e.g., 0.95 = 95% of original rating)
...
for tf in tfs
    for (i, ldes_bus) in enumerate(buses)
        run_new_simulation("5bus_Wind_b$(i)_line$(tf)_PTDF", ldes_bus, tf)
    end
end