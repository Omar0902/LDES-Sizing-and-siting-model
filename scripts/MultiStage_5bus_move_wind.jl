...
output_dir = (@__DIR__)*"/5bus_sims/move_wind"
...
function run_new_simulation(sim_name, bus, vre_bus)
    ...
    if vre_bus in ["node_b", "node_c"]
        vre_to_switch = get_component(RenewableDispatch, sys_UC, Wind_rd_dict[vre_bus][1])
...
for (i, wind_bus) in enumerate(buses)
    for (ii, ldes_bus) in enumerate(buses)
        run_new_simulation("5bus_Wind_ld$(ii)_wind$(i)_PTDF", ldes_bus, wind_bus)
    end
end
