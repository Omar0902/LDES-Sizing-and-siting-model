...
output_dir = (@__DIR__)*"/5bus_sims/move_SDES_horizons"

solver_HiGHS = optimizer_with_attributes(
...
function run_new_simulation(sim_name, bus, bus_SDES, horizon, time_path; solver = solver_HiGHS)
    ...
    writedlm((@__DIR__)*"/5bus_sims/"*time_path, [exec_time], ',')
end

horizons = [48, 96, 15 * 24]
buses = ["node_a", "node_b", "node_c", "node_d", "node_e"]

for (i, h) in enumerate(horizons)
    for (ii, ldes_bus) in enumerate(buses)
        run_new_simulation("5bus_Wind_ld$(ii)_sd1_horizon$(h)_PTDF", ldes_bus, "node_a", h, "exec_time_ld$(ii)_sd1_horizon$(h)_time.csv", solver = solver_HiGHS)
    end
end
